...
class MvcController {
    ...
    public function set_object()
    {
        if (!empty($this->model->invalid_data))
        {
            ...
            {
                $this->model->invalid_data[$this->model->primary_key] = intval( $this->params['id'] );
            }
            $object = $this->model->new_object($this->model->invalid_data);
        } else if (!empty(intval( $this->params['id'] )))
        {
            $object = $this->model->find_by_id(intval($this->params['id']));
        }
